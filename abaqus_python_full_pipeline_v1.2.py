...
def get_average_HFL_nogui(odb_path, set_name, direction):
    ...
    """Extract the average HFL component on a given node set from odb (unweighted average)"""
...
# ============================================================
# 5. Master control: one-shot pipeline
# ============================================================
...
def full_pipeline(stp_path,
    ...
    """
    One-shot: STP → INP → ODB → HFL → keff
    - L: specimen length in the heat flow direction (same units as model)
    - deltaT: temperature difference between top and bottom = Thot - Tcold
    """
    ...
    # 2. INP → ODB (steady-state heat transfer)
    ...
    print("avg_HFL: {}, keff: {}".format(avg_HFL, keff))
    print("Effective thermal conductivity keff =", keff)


    # 4. Append result to csv
    result_csv = '/public/home/nieqi01/zhy/abaqus_hfl_test/keff_results.csv'
    try:
        line = "%s,%s,%g,%g,%g\n" % (
...
if __name__ == '__main__':
    ...
    default_stp = "/public/home/nieqi01/zhy/abaqus_hfl_test/137777.stp"
    default_inp = "/public/home/nieqi01/zhy/abaqus_hfl_test/137777-symmetryMesh.inp"
    ...
    # Heat transfer direction & specimen length (L ≈ unit-cell size * array count)
    direction = 'y'
    L = 10.0   # e.g., 10 mm
    Thot = 30.0
    Tcold = 25.0
    ...
    part_name = 'FINALINSTANCE'
    instance_name = 'FINALINSTANCE-1'
    set_name = 'YTOP'  # top surface corresponding to direction='y'
